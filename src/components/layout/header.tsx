...
import { Menu, Palette, Search, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { ThemeSwitcher } from "@/components/layout/theme-switcher";
import { LibrariesNav } from "@/components/layout/libraries-nav";
import { HEADER_LIBRARY_LINKS } from "@/lib/nav";
import {
  Dialog,
  DialogContent,
  DialogHeader,
    ...
    <header className="sticky top-0 z-40 border-b border-border/50 bg-background/80 pt-[env(safe-area-inset-top)] backdrop-blur-xl supports-[backdrop-filter]:bg-background/70">
      <div className="mx-auto flex h-14 max-w-[1600px] items-center gap-2 px-3 sm:h-16 sm:gap-3 sm:px-5 lg:px-8">
              ...
              <div className="space-y-5 p-3 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
                <nav aria-label="Primary" className="space-y-1">
                  <Link
                    ...
                    className="flex items-center gap-3 rounded-2xl bg-rose-600 px-3 py-3 text-sm font-medium text-white hover:bg-rose-700"
                  >
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          >
            <Link href="/ai-color-copilot" aria-label="Copilot">
              <Sparkles className="h-4 w-4" />
            </Link>
          </Button>
          <ThemeSwitcher />
